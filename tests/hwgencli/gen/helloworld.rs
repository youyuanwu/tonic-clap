...
impl GreeterCommands {
    async fn execute(
        ...
        let mut c = greeter_client::GreeterClient::new(ch);
        match self {
            GreeterCommands::SayHello => {
                let request: HelloRequest = match json_data {
            ...
            GreeterCommands::SayHello2 => {
                let request: HelloRequest2 = match json_data {
                    Some(data) => serde_json::from_str(&data).unwrap(),
                    None => Default::default(),
                };
...
impl Greeter2Commands {
    async fn execute(
        ...
        let mut c = greeter2_client::Greeter2Client::new(ch);
        match self {
